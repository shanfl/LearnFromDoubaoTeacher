...
int main() {
    ...
    int w = 1280, h = 720;
    GLFWwindow* win = glfwCreateWindow(w, h, "Forward+ 真实3D光照", NULL, NULL);
    glfwMakeContextCurrent(win);
    gladLoadGLLoader((GLADloadproc)glfwGetProcAddress);
    glViewport(0, 0, w, h);
    glEnable(GL_DEPTH_TEST);

    int tilesX = w / TILE_SIZE;
    int tilesY = h / TILE_SIZE;
    ...
    for (int i = 0; i < MAX_LIGHTS; ++i) {
        vec3 ws = vec3(rand() % 60 - 30, 3.0f, rand() % 60 - 30);
        lights[i].posVS = vec4(ws, 1);
        lights[i].color = vec4(0.2f, 0.2f, 0.2f, 1.0f);
        lights[i].radius = 22.0f;
        lights[i].pad[0] = lights[i].pad[1] = lights[i].pad[2] = 0;
    }

    // 前3个强彩色光源
    lights[0].color = vec4(0.9f, 0.2f, 0.2f, 1.0f);
    lights[1].color = vec4(0.2f, 0.9f, 0.2f, 1.0f);
    lights[2].color = vec4(0.2f, 0.2f, 0.9f, 1.0f);
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        // 更新光源到视空间
        for (int i = 0; i < MAX_LIGHTS; ++i) {
            vec4 worldPos = lights[i].posVS;
            vec4 viewPos = view * worldPos;
            lights[i].posVS = viewPos;
        }

        glBindBuffer(GL_UNIFORM_BUFFER, lightUBO);
        glBufferSubData(GL_UNIFORM_BUFFER, 0, MAX_LIGHTS * sizeof(Light), lights.data());
